...
/// Gets the width of a sprite from custom_size or the loaded image asset.
fn get_sprite_width(sprite: &Sprite, images: &Assets<Image>) -> f32 {
    sprite
        .custom_size
        .map(|s| s.x)
        .or_else(|| images.get(&sprite.image).map(|img| img.size().x as f32))
        .unwrap_or(0.0)
}
...
pub fn children_count_system(
    ...
    layer_query: Query<
        (Entity, &ChildOf, Option<&Children>, &Sprite, &Transform),
        With<Layer>,
    >,
) {
    for (entity, child_of, children, sprite, transform) in layer_query.iter() {
        if let Ok(window) = cameras_query.get(child_of.0) {
            let width = get_sprite_width(sprite, &images);
            ...
            for _ in 0..to_add {
                commands.spawn((
                    Sprite::from_image(sprite.image.clone()),
                    ChildOf(entity),
                ));
            }
